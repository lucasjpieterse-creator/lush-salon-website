...
export default function ManagerPage(){
  ...
  const getServiceName = (b:any) => {
    if(!b.notes) return 'Service'
    return b.notes.split('|')[0].trim() // "Manicure" not "Manicure | R150 | 45min..."
  }
  const getPrice = (b:any) => b.total_price || b.price || ''
  const getTimeFromNotes = (b:any) => {
    // notes has "... Wants: 2026-09-28 09:00"
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-2xl mx-auto p-6">
        ...
        <h2 className="font-black text-xl mt-8 mb-3">⏰ Pending</h2>
        <div className="grid gap-3">
            ...
            <div key={b.id} className="bg-zinc-900 border border-zinc-800 rounded-[20px] p-4">
              <div className="flex justify-between">
                <div>
                  ...
                  <p className="text-zinc-500 text-xs mt-0.5">{b.client_phone || b.customer_phone} • {b.booking_date} {getTimeFromNotes(b)}</p>
          ...
          {pending.length===0 && <p className="text-zinc-600 text-sm bg-zinc-900 border border-zinc-800 border-dashed rounded-2xl p-6 text-center">No pending bookings 🌙</p>}
        </div>
        ...
        <div className="grid gap-2 opacity-60">
          {confirmed.slice(0,10).map(b=>(
            <div key={b.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 flex justify-between text-sm">
              <span>{b.client_name} • {b.booking_date} {getTimeFromNotes(b)} • {getServiceName(b)}</span><span className="text-green-400 font-bold">✓</span>
            </div>
          ))}
        </div>
      </div>
