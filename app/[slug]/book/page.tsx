...
export default function BookPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // auto-select service from URL like?service=Wash & Blow
  ...
  const handleBooking = async () => {
    ...
    // Save to bookings table for dashboard/income
...
*Stylist:* ${selectedStylist.name} - ${selectedStylist.specialty}
*Date:* ${date}
*Time:* ${selectedTime}
...
Please confirm my slot 🙏`
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 max-w-3xl mx-auto pb-32">
      ...
      <h2 className="font-bold mt-8 mb-3">1. Service {serviceFromUrl && `(Selected: ${serviceFromUrl})`}</h2>
      ...
      <h2 className="font-bold mt-8 mb-3">2. Choose Stylist ({stylists.length}) - Specialty</h2>
        ...
        {stylists.map(st=>(
          <button key={st.id} onClick={()=>setSelectedStylist(st)} className={`p-4 rounded-2xl border text-left flex gap-3 items-center ${selectedStylist?.id===st.id?'bg-white text-black':'bg-zinc-900 border-zinc-800'}`}>
            ...
            <div className="flex-1">
              <p className="font-bold">{st.name} {st.name==="Thandi" && "• Open at 15:00"}</p>
              <p className="text-xs opacity-70">{st.specialty}</p>
              {st.name==="Thandi" && <p className="text-[10px] mt-1 bg-[#25D366] text-black inline-block px-2 py-0.5 rounded-full font-bold">BRAIDS SPECIALIST - 15:00 SLOT</p>}
            </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {times.map(t=>(
          <button key={t} onClick={()=>setSelectedTime(t)} className={`py-3 rounded-xl border font-bold ${selectedTime===t?'bg-[#25D366] text-black border-[#25D366]':'bg-zinc-900 border-zinc-800'}`}>
            {t}
            {selectedStylist?.name==="Thandi" && t==="15:00" && <span className="block text-[10px]">RECOMMENDED</span>}
          </button>
        ))}
      </div>
      ...
      {selectedService && selectedStylist && selectedTime && (
        <div className="mt-8 p-6 bg-white text-black rounded-[2rem] sticky bottom-6 shadow-2xl">
          ...
          <button onClick={handleBooking} className="w-full mt-4 bg-black text-white py-4 rounded-2xl font-bold text-center">
            Confirm on WhatsApp →
          </button>
          <p className="text-[10px] text-zinc-500 mt-2 text-center">Booking will be saved to dashboard + WhatsApp</p>
        </div>
      )}
    </div>
  )
